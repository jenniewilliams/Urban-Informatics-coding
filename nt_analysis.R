...
favorites('NT_Distrust')
spRead <- getUser('NT_Distrust')
sp_follower_IDs<-spRead$getFollowers(retryOnRateLimit=180)
sp_followers_df <- twListToDF(sp_follower_IDs)
sp_followers_df$friendsCount<-sp_followers_df$friendsCount/1000
sp_followers_df$followersCount<-sp_followers_df$followersCount/1000

sp_followers_df <- subset(sp_followers_df, followersCount >= 50)


plot(sp_followers_df$friendsCount, sp_followers_df$followersCount, main='Friends and Followers of @NT_Distrust.', xlab='Friends(Thousands)', ylab='Followers(Thousands)')
text(sp_followers_df$friendsCount, sp_followers_df$followersCount, labels=sp_followers_df$screenName, cex= 0.6, pos=3)

# Get 250 tweets from the Speed Reads
sp<-userTimeline('NT_Distrust', n=250)
...
#sp2[124]
# substitute R_encoding for Description
...
# writeLines(as.character(corp[[124]]))
...
# For SpeedReads
wordcloud(df0$newterm,df0$freq,c(5,.4),2,100,TRUE,,.15,pal,vfont=c("serif","plain"))
